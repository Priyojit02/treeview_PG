    ...
    <TreeItem
      ...
      sx={{
        "& .MuiTreeItem-content": {
          ...
          ...(isHovered && {
            backgroundColor: alpha(useTheme().palette.primary.main, 0.06),
          }),
          ...(isSelected && {
        ...
        "& .MuiTreeItem-group": {
          marginLeft: 1.25,
          paddingLeft: 1.0,
          borderLeft: `1px dashed ${useTheme().palette.divider}`,
        },
      }}
    >
      {kids.map((child) => (
...
export default function App() {
  ...
  // Fetch tree (now supports filters via query params)
            ...
            <Grid item xs={12} md={6} lg={7}>
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, height: "100%" }}>
                ...
                <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems="center" sx={{ mb: 1 }}>
                  ...
                  <TextField size="small" label="End period" value={filters.endPeriod} onChange={(e) => setFilters((s) => ({ ...s, endPeriod: e.target.value }))} sx={{ minWidth: 110 }} />

                  <TextField size="small" label="Comparison period" value={filters.comparisonEndPeriod} onChange={(e) => setFilters((s) => ({ ...s, comparisonEndPeriod: e.target.value }))} sx={{ minWidth: 140 }} />
